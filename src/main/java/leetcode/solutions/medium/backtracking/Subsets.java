...
public class Subsets {
    ...
    private void dfs(int[] nums, int i, List<Integer> subset, List<List<Integer>> res) {
        if (i == nums.length) {
            ...
            // The reason why we need a copy is that ArrayList is a reference type, and as such its reference would still 
            // be accessible in other recursive calls (e.g. when we pop from it), but we do not want that.
